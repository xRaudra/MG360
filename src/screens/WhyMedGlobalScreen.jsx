    ...
    <div className="flex flex-col h-full bg-transparent screen-enter">
      ...
      <div className="flex-1 overflow-y-auto hide-scrollbar"
        ...
        <div className="px-4 py-5 flex flex-col gap-4">
          ...
          {/* Mission */}
          <div className="rounded-2xl p-4" style={{ background: 'linear-gradient(135deg, #EFF6FF, #F0FDFA)', border: '1px solid #C6C6C6' }}>
            <p className="text-xs font-bold uppercase tracking-wider mb-2" style={{ color: '#1B4FBF' }}>Our Mission</p>
          ...
          <div className="flex gap-2">
            {stats.map(s => (
              <div key={s.label} className="rounded-2xl py-3 px-1 text-center flex-1" style={CARD_STYLE}>
                <p className="font-bold text-base leading-none" style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', color: '#1B4FBF' }}>{s.value}</p>
          ...
          {/* Why us */}
          <div>
            <p className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: '#7C7C7C' }}>Why Choose Us</p>
            <div className="flex flex-col gap-3">
              {pillars.map(p => {
                const Ic = p.icon;
                return (
                  <div key={p.title} className="flex items-start gap-3 rounded-2xl p-4" style={CARD_STYLE}>
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
                      style={{ background: p.bg }}>
                      <Ic size={18} color={p.color} />
                    </div>
                    <div>
                      <p className="font-semibold text-sm" style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', color: '#313131' }}>{p.title}</p>
                      <p className="text-xs mt-0.5 leading-relaxed" style={{ color: '#7C7C7C' }}>{p.desc}</p>
                    </div>
                  </div>
                );
              })}
          ...
          {/* How it works */}
          <div>
            <p className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: '#7C7C7C' }}>How It Works</p>
            <div className="rounded-2xl overflow-hidden" style={CARD_STYLE}>
              {howItWorks.map((item, i) => (
                <div key={item.step} className="flex items-start gap-3 px-4 py-3.5"
                  style={{ borderBottom: i < howItWorks.length - 1 ? '1px solid #F0F0F0' : 'none' }}>
                  ...
                  <div>
                    <p className="font-semibold text-sm" style={{ color: '#313131' }}>{item.title}</p>
                    <p className="text-xs mt-0.5" style={{ color: '#7C7C7C' }}>{item.desc}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div>
            ...
            <button onClick={() => onNavigate('whyIndia')}
              ...
              <div className="flex-1">
                <p className="font-semibold text-sm" style={{ color: '#313131' }}>Why India?</p>
                <p className="text-xs" style={{ color: '#7C7C7C' }}>The world's leading medical tourism destination</p>
              </div>
